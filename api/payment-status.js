import { CheckoutError, checkoutErrorPayload, checkoutProductAllowsConsumedAccess, checkoutProductNeedsLedger, verifyMercadoPagoPayment } from "../server/checkout-core.js";
...
function parseBody(request) {
  const body = request.body && typeof request.body === "object" ? request.body : typeof request.body === "string" ? JSON.parse(request.body) : null;
...
export default async function handler(request, response) {
  ...
  try {
    const result = await verifyMercadoPagoPayment(body);
    if (checkoutProductNeedsLedger(result.entitlement.productId)) {
      try {
        ...
        const reusable = ledger.state === "consumed" && checkoutProductAllowsConsumedAccess(result.entitlement.productId);
        if (ledger.state !== "active" && !reusable) throw new CheckoutError("payment_credit_unavailable", 409);
        result.entitlement.state = ledger.state;
        result.entitlement.creditAvailable = ledger.state === "active";
        result.entitlement.completeSummaryUsed = ledger.completeSummaryUsed === true;
        result.entitlement.includedQuestionsUsed = Math.max(0, Number(ledger.includedQuestionsUsed) || 0);
      } catch (error) {
        if (error instanceof CheckoutError) throw error;
        if (error instanceof PaymentLedgerError) throw new CheckoutError(error.code, error.status);
        throw error;
      }
    }
