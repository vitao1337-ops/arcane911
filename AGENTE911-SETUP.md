# Agente 911 V11 — Gemini na Vercel e no localhost

A V11 usa Gemini no mesmo endpoint server-side `/api/agent-911`. Nenhuma chave entra no React. O fluxo conectado nasce ativo, tenta `gemini-3.5-flash` e passa para `gemini-3.5-flash-lite` quando o primeiro modelo atinge limite ou fica indisponível. A interface espera essa resposta conectada: não mostra uma leitura local provisória enquanto o Gemini trabalha.
...
- `GEMINI_MODEL` e `GEMINI_FALLBACK_MODEL` são opcionais; os valores acima já são os padrões da V11.
...
Suba a V11 no repositório já conectado ao projeto atual. Não crie outro projeto na Vercel. O `vercel.json` preserva a função e as rotas React.
...
6. Para testar resiliência, altere temporariamente `GEMINI_API_KEY` em um Preview, faça redeploy e confirme que aparece a opção discreta de tentar novamente, sem texto de tarot automático e sem bloco vermelho; não faça esse teste destrutivo em Production.
...
O Vite encaminha `/api` para `https://arcane911.vercel.app`. Portanto, depois que a V11 estiver publicada, o localhost usa exatamente a mesma função e a chave que já está na Vercel. A chave não precisa existir no computador e nunca chega ao navegador.
