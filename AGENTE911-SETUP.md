# Agente 911 V12 — Gemini na Vercel e no localhost

A V12 usa Gemini no mesmo endpoint server-side `/api/agent-911`. Nenhuma chave entra no React. O fluxo conectado nasce ativo, tenta `gemini-3.5-flash` e passa para `gemini-3.5-flash-lite` quando o primeiro modelo atinge limite ou fica indisponível. A interface espera essa resposta conectada: não mostra uma leitura local provisória enquanto o Gemini trabalha. A chave de postura escolhida antes da tiragem também chega ao servidor e acompanha toda a Ferradura e a Consulta 911.
...
- `GEMINI_MODEL` e `GEMINI_FALLBACK_MODEL` são opcionais; os valores acima já são os padrões da V12.
...
Suba a V12 no repositório já conectado ao projeto atual. Não crie outro projeto na Vercel. O `vercel.json` preserva a função e as rotas React.
...
3. Repita com **Direta** e **Sem rodeios**; numa pergunta binária, confirme o selo **SIM**, **NÃO** ou **INCONCLUSIVA**.
...
7. Para testar resiliência, altere temporariamente `GEMINI_API_KEY` em um Preview, faça redeploy e confirme que aparece a opção discreta de tentar novamente, sem texto de tarot automático e sem bloco vermelho; não faça esse teste destrutivo em Production.
...
O Vite encaminha `/api` para `https://arcane911.vercel.app`. Portanto, depois que a V12 estiver publicada, o localhost usa exatamente a mesma função e a chave que já está na Vercel. A chave não precisa existir no computador e nunca chega ao navegador.
